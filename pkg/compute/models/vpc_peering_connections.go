...
func (manager *SVpcPeeringConnectionManager) ListItemFilter(
	...
	if len(query.VpcId) > 0 {
		...
		q = q.In("vpc_id", vpc.GetId())
	}
	if len(query.PeerVpcId) > 0 {
		q = q.In("peer_vpc_id", query.PeerVpcId)
	}
	return q, nil
}
...
func (manager *SVpcPeeringConnectionManager) ValidateCreateData(
	...
	if len(input.VpcId) < 0 {
		return input, httperrors.NewInputParameterError("vpc_id: missing input field")
	}
	vpcPC := SVpcPeeringConnection{}
	err = manager.Query().Equals("vpc_id", input.VpcId).Equals("peer_vpc_id", input.PeerVpcId).First(&vpcPC)
	if err == nil {
	...
	err = manager.Query().Equals("vpc_id", input.PeerVpcId).Equals("peer_vpc_id", input.VpcId).First(&vpcPC)
	if err == nil {
		return input, httperrors.NewNotSupportedError("vpc %s and vpc %s have already connected", input.VpcId, input.PeerVpcId)
	} else {
		if errors.Cause(err) != sql.ErrNoRows {
	...
	if account.Provider != peerAccount.Provider {
		return input, httperrors.NewNotSupportedError("vpc peering for different cloudprovider is not supported")
	}

	factory, err := cloudprovider.GetProviderFactory(account.Provider)
	if err != nil {
		return input, httperrors.NewGeneralError(errors.Wrap(err, "cloudprovider.GetProviderFactory"))
	}
	CrossCloud := account.AccessUrl != peerAccount.AccessUrl
	CrossRegion := vpc.CloudregionId != peerVpc.CloudregionId
	if !factory.IsSupportVpcPeering(CrossCloud, CrossRegion) {
		return input, httperrors.NewNotSupportedError("cloudprovider %s %s %s %s %s not supported vpcpeering", account.Provider, account.AccessUrl, vpc.CloudregionId, peerAccount.AccessUrl, peerVpc.CloudregionId)
	}
	return input, nil
}
...
func (self *SVpcPeeringConnection) StartDeleteVpcPeeringConnectionTask(ctx context.Context, userCred mcclient.TokenCredential) error {
	...
	if err != nil {
		log.Errorf("Start VpcPeeringConnectionDeleteTask fail %s", err)
		return err
	}
	task.ScheduleRun(nil)
	return nil
...
func (self *SVpcPeeringConnection) PerformSyncstatus(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.VpcSyncstatusInput) (jsonutils.JSONObject, error) {
	return self.PerformSync(ctx, userCred, query, input)
}

func (self *SVpcPeeringConnection) AllowPerformSync(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, data jsonutils.JSONObject) bool {
	return db.IsAdminAllowPerform(userCred, self, "sync")
}

func (self *SVpcPeeringConnection) PerformSync(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.VpcSyncstatusInput) (jsonutils.JSONObject, error) {
