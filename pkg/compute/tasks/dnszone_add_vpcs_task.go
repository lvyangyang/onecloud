	...
	for i := range caches {
		if len(caches[i].ExternalId) == 0 {
			for _, vpcId := range vpcIds {
				err = dnsZone.AddVpc(ctx, vpcId)
				if err != nil {
					self.taskFailed(ctx, dnsZone, errors.Wrapf(err, "dnsZone.AddVpc(%s)", vpcId))
					return
				}
			}
		...
		for _, vpcId := range vpcIds {
			...
			if err != nil {
				self.taskFailed(ctx, dnsZone, errors.Wrapf(err, "iDnsZone.AddVpc(%s,%s)", opts.Id, opts.RegionId))
				return
			}
			err = dnsZone.AddVpc(ctx, vpcId)
			if err != nil {
