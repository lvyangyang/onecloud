...
func (self *SVpcPeeringConnection) Refresh() error {
	vpcPCs, err := self.vpc.region.DescribeVpcPeeringConnections(self.vpc.VpcId)
	if err != nil {
		return errors.Wrapf(err, "self.region.DescribeVpcPeeringConnections()")
	}
	for i := range vpcPCs {
		if vpcPCs[i].VpcPeeringConnectionId != nil && self.vpcPC.VpcPeeringConnectionId != nil {
			if *vpcPCs[i].VpcPeeringConnectionId == *self.vpcPC.VpcPeeringConnectionId {
				self.vpcPC = vpcPCs[i]
				return nil
			}
		}
...
func (self *SVpcPeeringConnection) GetRequestVpcId() string {
	if self.vpcPC.AccepterVpcInfo != nil {
		if self.vpcPC.RequesterVpcInfo.VpcId != nil {
			return *self.vpcPC.RequesterVpcInfo.VpcId
		}
	}
	return ""
}
